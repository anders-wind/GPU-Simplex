...
-- tags { full_parallel }
--
-- compiled input @generated_tests/full_test_in.txt
-- output @generated_tests/full_test_out.txt
...
let multi_simplex [h] (As:[]f32) (bs:[]f32) (cs:[]f32) (ms:[h]i32) (ns:[h]i32) =
  ...
  let (_,_,_,vs,_,_)    = loop (As, bs, cs, vs, es, continue) while continue do
    -- unroll loop 5 times to avoid futhark double buffer memcpy overhead
      ...
      in if continue then
        ...
        in if continue then
          ...
          in if continue then
            ...
            let es        = entering_variables flag_n iota_ns ns_scan cs
            let continue  = reduce (\res e -> res || e) (false) (map(\e -> e != (-1)) es)
            in (As,bs,cs,vs,es,continue)
          else (As,bs,cs,vs,es,continue)
        else (As,bs,cs,vs,es,continue)
      else (As,bs,cs,vs,es,continue)
    else (As,bs,cs,vs,es,continue)
  in vs
