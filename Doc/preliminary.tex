...
One way to represent the constraints and variables is with vectors and matrices. Since we have a $n$ variables the coefficients of the variables can be encoded in a vector $c$, there are $m$ constants of the constraints which can be encoded into a vector $b$ and the constraints can be seen as a matrix $A$ over every variable and constraint row. The sequential simplex algorithm is the basis for the implementations of this project is based on a compact version of the algorithm presented in "Introduction to Algorithms"\footnote{Third edition, Chapter 29} by Thomas H. Cormen et al.
...
\begin{table}[H]
	...
	\end{tabular}
	\caption{Matrix representation of a linear program with 3 variables and 2 constraints. The blue cells coresponds to the c vector. The red cells corresponds to the constraint matrix and the green cells correspond to the constants vector.}
\end{table}